    ...
    public partial class FinishedRestoreSetup : WizardControl
    {
        ...
        private void Restore(object sender, DoWorkEventArgs args)
        {
            ...
            using (Library.Utility.TempFolder tf = new Duplicati.Library.Utility.TempFolder())
            {
                ...
                // TODO: Issue 331: Restore setup keeps the local signature path and temporary folder settings
                // if appset.TempPath or appset.SignatureCachePath are not valid paths, set them to empty value
                // so they are reinitialized with default values on next start
